...
Think of metrics in four tiers. Each tier is more meaningful and harder to measure than the one below it. A healthy program reports mostly on tiers 2–3 and uses tier 1 only for early-stage plumbing checks.
...
- **What it's good for:** detecting Stage 0→1 problems (nobody's using it → access/permission/fear issue) and *retention* is a weak proxy for value (people abandon tools that don't help).
...
- **Baseline first.** Capture 2–4 weeks of the old workflow's numbers before touching it. Retrospective baselines are fiction.
