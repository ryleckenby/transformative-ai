...
- recurs frequently (daily/weekly, not quarterly), because frequency is what compounds
...
For 2 to 4 weeks, run the new path alongside the old one with the champion and 2 to 3 volunteers.
...
- Expect the dip: weeks 1 and 2 are often *slower* as habits rewire. Tell everyone this in advance so nobody calls the pilot dead at day 10.

## Phase 5: Harden or kill

At the end of the pilot window, make an explicit call:

- **Harden:** the win is real, so productionize. Add monitoring, expand the eval set, document the workflow, train the rest of the team, and *retire the old path* (a new path that coexists indefinitely with the old one will lose, because the old path is muscle memory).
- **Kill:** the win isn't there, so say so loudly and in writing. A clean, documented kill builds more credibility than a zombie pilot. Capture *why* (model capability? data access? verification too expensive?); today's kill is often next year's win when capability improves.
