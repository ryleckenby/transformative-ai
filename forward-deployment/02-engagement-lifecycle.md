...
How a single forward-deployment engagement runs, from qualification to handoff. Written to work for both arenas (vendor to customer, and internal platform to business team), with notes where they differ. Timeboxes are typical for an AI workflow engagement; scale to taste.
...
- **In their systems, not beside them.** The output lands in the ticket queue, CRM, or document repo they already live in.
...
- **The permanent engagement.** No end state in the charter, so the FDE becomes staff augmentation. The fix is always in the charter.
...
- **Skipping the baseline.** No before-measurement means no defensible claim, and the renewal or expansion conversation runs on anecdote.
